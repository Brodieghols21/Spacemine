...
public class ShrinkLaser3Block extends Block {
    ...
    public ShrinkLaser3Block(Properties properties) {
        super(properties);
    }
    ...
    public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        ...
        boolean isNextTier = stack.isItemEqual(new ItemStack(ItemRegistry.MULTIVERSE.get()));
        if (state.getBlock().getDefaultState().equals(BlockRegistry.SHRINK_LASER.get().getDefaultState()) && isNextTier) {
            worldIn.setBlockState(pos, BlockRegistry.SHRINK_LASER_4.get().getDefaultState());
    ...
    public void tick(BlockState state, ServerWorld worldIn, BlockPos pos, Random rand) {
        ...
        Item item = ItemRegistry.BLACK_HOLE.get();
        if(Math.random()<.0025)
